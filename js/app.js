...
function scrollRefresh(divID, flag)
{
  if(ENV.device.touchSupport){
    ...
    if(flag){
      scrolls[divID].scrollTo(0,0,false);
    }
  }
}
...
var Canvas = {
  current: null,
    ...
    Canvas.current = theCanvas;
        ...
        scrollRefresh(App.maincanvas);
        continue;
      }

			if(App.canvases[i] === theCanvas){
				Canvas.doTransition("in", App.canvases[i]);
        if(App.scrolls.indexOf(App.canvases[i]) >=0){
    ...
    if(theCanvas!=null){
      if(ENV.device.touchSupport){
         scrolls[theCanvas].scrollTo(0,0,0);
      }
      else{
         scrolls[theCanvas].scrollTo(0,0,false);
      }
    }
	},
...
function loadCanvasSkeleton(data)
{
  ...
  if(ENV.device.touchSupport){
    getElementObject("body").className = "touchSupport";
  }
  else{
    getElementObject("body").className = "web";
  }
...
{   
  ...
  if(App.scrolls.indexOf(canvasID) >=0){
    if(ENV.device.touchSupport){
...
function resizeend() {
    if (new Date() - rtime < delta) {
        ...
        timeout = false;
        //alert('Done resizing');
    var temp = ENV.screen.smallscreenon;
    setupEnvironment();
    ENV.screen.smallscreenon = temp;
    if(SlideShow.active){
      setTimeout('SlideShow.resize()', 1000)
    }
		
    var temp1 = Canvas.current;
    console.log(temp1);
    Canvas.showOneCanvasOnly(null);
    setTimeout('Canvas.showOneCanvasOnly("' + temp1 +  '")', 700);
    setTimeout('fadeOut("splashScreen")', 1300);
    }               
}
...
function initializeApp() {  
  getElementObject("poweredbyText").innerHTML = "Resizing ...";
	var i;
	...
	setTimeout('document.getElementById("splashScreen").style.opacity = "0";', 2000);
...
var messagesPlugin = {
  ...
  hide: function(){
    fadeOut("messagesPluginScreen");
  },
  initialize:function(){
    FNS.addButtonElement("mpPluginDone", messagesPlugin.doneClicked);
    FNS.addButtonElement("mpPluginComments", messagesPlugin.commentsClicked);
    FNS.addButtonElement("mpPluginShare", messagesPlugin.shareClicked);
    FNS.addButtonElement("mpPluginFav", messagesPlugin.favClicked);
    FNS.addButtonElement("mpPluginPrev", messagesPlugin.prevClicked);
    FNS.addButtonElement("mpPluginNext", messagesPlugin.nextClicked);
    //$("#messagesPluginCWW").jScrollPane();
  },
